...
// xdivs - number of cell divisions to use in vertical (x) direction (MIN=2 MAX = 16) xdiv must be an integer multiple of image width
// ydivs - number of cell divisions to use in vertical (y) direction (MIN=2 MAX = 16) ydiv must be an integer multiple of image height
...
void cvCLAdaptEqualize(IplImage *src, IplImage *dst, float limit)
{
...
const static unsigned int uiMAX_REG_X = 64;      /* max. # contextual regions in x-direction */
const static unsigned int uiMAX_REG_Y = 64;      /* max. # contextual regions in y-direction */

/************************** main function CLAHE ******************/
static int CLAHE (kz_pixel_t* pImage, unsigned int uiXRes, unsigned int uiYRes, kz_pixel_t Min, kz_pixel_t Max, 
				  unsigned int uiNrX, unsigned int uiNrY, unsigned int uiNrBins, float fCliplimit)
 ...
 */
{
	...
	long minClip=(uiXSize * uiYSize) / uiNrBins;

    if(fCliplimit > 0.0) {                /* Calculate actual cliplimit  */
       ulClipLimit = (unsigned long) (minClip+fCliplimit*(ulNrPixels- minClip));
...
void MakeHistogram (kz_pixel_t* pImage, unsigned int uiXRes, unsigned int uiSizeX, unsigned int uiSizeY,
